...
const Sessions: React.FC<SessionsProps> = ({ language, pendingSessionKey, onSessionKeyConsumed }) => {
  ...
  const extractConfigObject = useCallback((data: any): Record<string, any> | null => {
    ...
    if (data.models || data.agents || data.gateway) return data;
    return null;
  }, []);

  const buildModelOptionsFromConfig = useCallback((cfg: Record<string, any>): Array<{ value: string; label: string }> => {
    const providers = cfg?.models?.providers;
    if (!providers || typeof providers !== 'object') return [];
    const out: Array<{ value: string; label: string }> = [];
    for (const [providerId, providerCfg] of Object.entries(providers as Record<string, any>)) {
      const models = Array.isArray(providerCfg?.models) ? providerCfg.models : [];
      for (const item of models) {
        const rec = typeof item === 'string' ? { id: item, name: item } : (item || {});
        ...
        out.push({ value: fullId, label: `${name} (${providerId})` });
      }
    }
    return Array.from(new Map(out.map((x) => [x.value, x])).values());
  }, []);
  ...
  const loadModelOptions = useCallback(async () => {
    ...
    try {
      const res = await gwApi.configGet();
      const cfg = extractConfigObject(res);
      const options = cfg ? buildModelOptionsFromConfig(cfg) : [];
  ...
  }, [buildModelOptionsFromConfig, extractConfigObject, gwReady]);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('axonclaw-chat-drafts');
  ...
  const saveDraft = useCallback((key: string, text: string) => {
    ...
    try { localStorage.setItem('axonclaw-chat-drafts', JSON.stringify(draftsRef.current)); } catch { /* ignore */ }
  }, []);
  ...
  useEffect(() => {
    const currentModel = String(activeSession?.model || '').trim();
    if (!currentModel) {
      setSelectedModel('');
      return;
    }
    ...
    setModelOptions((prev) => (
      prev.some((opt) => opt.value === currentModel)
        ? prev
        : [{ value: currentModel, label: currentModel }, ...prev]
    ));
  }, [activeSession?.model, sessionKey]);

  const handleModelChange = useCallback(async (value: string) => {
    const next = value.trim();
    const prev = String(activeSession?.model || '').trim();
    setSelectedModel(next);
    if (!sessionKey || next === prev) return;
    setModelApplying(true);
    try {
      await gwApi.proxy('sessions.patch', {
        key: sessionKey,
        model: next || null,
      });
      setSessions((old) => old.map((s) => (s.key === sessionKey ? { ...s, model: next } : s)));
      toast('success', c.modelSwitchOk || c.quickModel);
    } catch (err: any) {
      setSelectedModel(prev);
      toast('error', err?.message || c.modelSwitchFailed || c.error);
    } finally {
      setModelApplying(false);
    }
  }, [activeSession?.model, c.error, c.modelSwitchFailed, c.modelSwitchOk, c.quickModel, sessionKey, toast]);
        ...
        <div className="p-3 md:p-4 shrink-0 border-t border-slate-100 dark:border-white/5 bg-white/80 dark:bg-[#0d1117]/80 backdrop-blur-xl">
          <div className="max-w-4xl mx-auto relative">
            ...
            <div className="flex items-center gap-2 px-1 pb-1.5">
              <span className="material-symbols-outlined text-[14px] text-slate-400 dark:text-white/35">smart_toy</span>
              <span className="text-[10px] font-bold text-slate-500 dark:text-white/35 uppercase tracking-wider shrink-0">
                {c.modelSelector || c.model}
              </span>
              <select
                value={selectedModel}
                onChange={(e) => { void handleModelChange(e.target.value); }}
                disabled={!gwReady || modelApplying}
                className="h-7 min-w-0 flex-1 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-[#121722] px-2 text-[11px] text-slate-700 dark:text-white/80 focus:outline-none focus:ring-1 focus:ring-primary/30"
              >
                <option value="">{c.modelAuto || c.model}</option>
                {modelOptions.map((opt) => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
              {(modelLoading || modelApplying) && (
