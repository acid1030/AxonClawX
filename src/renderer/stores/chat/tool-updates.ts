import { extractTextFromContent, isToolResultRole } from './message-content';
...
function extractToolResultUpdate(message: unknown, eventState: string): ToolStatus | null {
  ...
  const durationMs = parseDurationMs(details?.durationMs ?? details?.duration ?? msg.durationMs);
