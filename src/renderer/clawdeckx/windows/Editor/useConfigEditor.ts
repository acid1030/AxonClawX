...
export function useConfigEditor(): UseConfigEditorReturn {
  ...
  const extractConfig = useCallback((data: any): Record<string, any> | null => {
    if (!data) return null;
    // Error response - don't treat as config
    if (data.error && typeof data.error === 'string' && !data.config && !data.parsed) {
      return null;
    }
    // local mode: { config: {...}, path: "...", parsed: true }
    if (data.config && typeof data.config === 'object') return data.config;
    // remote mode: { parsed: {...} } - Gateway config.get with parsed field
    if (data.parsed && typeof data.parsed === 'object') return data.parsed;
    // raw JSON string mode: { raw: "{...}" } - parse it
    ...
    // direct config object (must have at least one known config key to avoid treating error objects as config)
    if (typeof data === 'object' && !Array.isArray(data)) {
      const knownConfigKeys = ['gateway', 'agents', 'models', 'channels', 'session', 'messages', 'tools', 'commands', 'hooks', 'cron', 'skills', 'plugins', 'extensions', 'memory', 'audio', 'browser', 'logging', 'auth', 'ui', 'update', 'web', 'discovery'];
      const hasKnownKey = knownConfigKeys.some(key => key in data);
      if (hasKnownKey) return data;
    }
    return null;
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Gateway 可能在配置不存在时返回字符串（如 "Not Found"），需要检测并降级
        if (typeof data === 'string') {
          console.log('[ConfigEditor] gwApi.configGet() returned string:', data);
          throw new Error('Gateway returned non-object response');
        }
        console.log('[ConfigEditor] gwApi.configGet() response:', JSON.stringify(data, null, 2).slice(0, 500));
        if (data?.hash) baseHashRef.current = data.hash;
        gwApi.configSchema().then((s: any) => setSchema(s)).catch(() => {});
      } catch (wsErr: any) {
        console.log('[ConfigEditor] gwApi.configGet() failed:', wsErr?.message || wsErr);
        // WS 不可用（网关未连接或返回无效数据），降级读本地文件
        try {
          data = await configApi.get();
          console.log('[ConfigEditor] configApi.get() response:', JSON.stringify(data, null, 2).slice(0, 500));
          if (data?.path) setConfigPath(data.path);
        } catch (localErr: any) {
          // 本地文件也读取失败
          const errCode = localErr?.code || (localErr?.message?.includes('not found') ? 'CONFIG_NOT_FOUND' : '');
          if (errCode) setLoadErrorCode(errCode);
          throw new Error(errCode === 'CONFIG_NOT_FOUND' ? 'Config file not found' : 'Gateway not connected and config file not found');
        }
      }
      const cfg = extractConfig(data);
      console.log('[ConfigEditor] extractConfig result:', cfg ? 'valid' : 'null', cfg ? Object.keys(cfg).slice(0, 10) : '');
      if (cfg) {
        setConfig(cfg);
        setDirty(false);
        setErrors([]);
        ...
        console.error('[ConfigEditor] Failed to extract config from:', typeof data, data);
        setLoadError('Failed to parse config data');
      }
    } catch (e: any) {
      console.error('[ConfigEditor] Load error:', e);
