  ...
  const handleServiceInstall = useCallback(async (service: 'openclaw' | 'clawdeckx') => {
    ...
    try {
      if (service === 'openclaw') {
        ...
        await serviceApi.installAxonClawX();
        toast('success', s.serviceInstalled || 'AxonClawX service installed');
      }
  ...
  const handleServiceUninstall = useCallback(async (service: 'openclaw' | 'clawdeckx') => {
    ...
    try {
      if (service === 'openclaw') {
        ...
        await serviceApi.uninstallAxonClawX();
        toast('success', s.serviceUninstalled || 'AxonClawX service uninstalled');
      }
      await loadServiceStatus();
    } catch (err: any) {
        ...
        <div className="px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-[13px] font-bold text-slate-700 dark:text-white/70">AxonClawX</h4>
              <SmartLink href="https://github.com/AxonClawX/AxonClawX" className="flex items-center text-slate-400 dark:text-white/30 hover:text-primary transition-colors" title="GitHub">
            ...
            <div className="mt-2 space-y-3">
              ...
              {!selfUpdating && !selfUpdateProgress?.done && (
                <div className="flex gap-2">
                  ...
                  <SmartLink href="https://github.com/AxonClawX/AxonClawX/releases"
        ...
        <div className={rowCls}>
          <div className="px-5 py-4">
            ...
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-slate-50 dark:bg-white/[0.03] text-[11px]">
              <span className="text-slate-400 dark:text-white/40">{s.updateCompatReq || 'AxonClawX requires OpenClaw'}:</span>
          ...
          {serviceStatus ? (
            <div className="space-y-2">
              ...
              <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-colors ${
                ...
                <span className="text-[16px]">🦀</span>
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-bold text-slate-700 dark:text-white/70">AxonClawX</p>
                  <p className="text-[10px] text-slate-400 dark:text-white/30 mt-0.5">
                    {serviceStatus.clawdeckx_installed
                      ? (s.serviceAutoStartEnabled || 'Auto-start enabled')
                      : (s.serviceAutoStartDisabled || 'Not registered, manual start required')}
