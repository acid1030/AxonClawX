...
async function request<T = any>(
  ...
  // AxonClaw Electron：/api/* 走 hostApiFetch，与 main 进程 hostapi:fetch 对接
  // preload 暴露的是 electron.ipcRenderer.invoke，不是 electron.invoke
  if (typeof window !== 'undefined' && (window as any).electron?.ipcRenderer?.invoke && url.startsWith('/api')) {
    try {
      const { hostApiFetch } = await import('@/lib/host-api');
      const method = (options.method as string) || 'GET';
      const result = await hostApiFetch<T>(url, {
        method,
      ...
      throw new ApiError(
        ...
        translateApiError(err.code || 'NETWORK_ERROR', err.message || 'Request failed'),
        err.status ?? 500
      );
    }
  }
